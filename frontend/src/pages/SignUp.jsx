...
import { AudioLines, UserPlus, ArrowLeft } from 'lucide-react';
    ...
    if (username && email && password) {
      ...
      try {
        ...
        if (error) {
          ...
          // If Supabase has email confirmation enabled, data.session might be null here
          if (data?.session) {
            navigate('/profile');
          } else {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-[420px] z-10">
        <div className="text-center mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <h1 className="text-3xl font-bold text-zinc-900 mb-2 tracking-wide">Create Account</h1>
          <p className="text-textSecondary text-sm">Join to start producing professional audio</p>
        </div>
        ...
        <div className="card animate-in fade-in zoom-in-95 duration-500 delay-150 fill-mode-both">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-2">
              <button type="submit" disabled={!username || !email || !password || loading} className="btn-primary w-full py-4 text-base flex justify-center items-center">
                {loading ? (
          ...
          <div className="mt-6 text-center text-sm text-textSecondary">
            ...
            <Link to="/signin" className="text-violet-400 hover:text-violet-800 font-medium transition-colors">
              Sign In
            </Link>
          </div>
        </div>
